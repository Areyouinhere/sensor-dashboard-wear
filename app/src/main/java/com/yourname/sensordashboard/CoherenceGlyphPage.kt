...
/* NOTE: This file must ONLY define the glyph page — no helpers duplicated from UiBits.kt */
...
fun CoherenceGlyphPage(readings: Map<String, FloatArray>) {
    ...
    val composite = CompassModel.composite.value  // keep in sync with model
    val conf = CompassModel.confidence.value

    Column(
        Modifier.fillMaxSize().padding(16.dp).verticalScroll(rememberScrollState())
    ) {
        Text("Coherence", fontSize = 18.sp, color = Color.White, modifier = Modifier.fillMaxWidth())
        ...
        Canvas(Modifier.fillMaxWidth().height(176.dp)) {
            ...
            fun ring(idx: Int, pct: Float, glow: Color, core: Color) {
                ...
                drawArc(
                    ...
                    topLeft = tl, size = sz, style = Stroke(width = 8f, cap = StrokeCap.Round)
                )
                // glow (alpha gated by confidence)
                drawArc(
                    color = glow.copy(alpha = 0.25f + 0.55f * conf),
                    startAngle = -90f, sweepAngle = 360f*pct, useCenter = false,
                    topLeft = tl, size = sz, style = Stroke(width = 10f, cap = StrokeCap.Round)
                )
                // core
                drawArc(
                    color = core,
                    startAngle = -90f, sweepAngle = (360f*pct).coerceAtLeast(6f), useCenter = false,
                    topLeft = tl, size = sz, style = Stroke(width = 5f, cap = StrokeCap.Round)
                )
            }
