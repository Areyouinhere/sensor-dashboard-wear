...
object CompassModel {
    ...
    fun pushHR(bpm: Float)       { rollHR.push(kotlin.math.max(0f, bpm)) }
    fun pushHRV(rmssd: Float)    { rollHRV.push(kotlin.math.max(0f, rmssd)) }
    fun pushGyroMag(m: Float)    { rollGyro.push(kotlin.math.max(0f, m)) }
    fun pushAccelMag(m: Float)   { rollAccel.push(kotlin.math.max(0f, m)) }
